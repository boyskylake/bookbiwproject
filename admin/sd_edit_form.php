...
$colname_sd = "-1";
if (isset($_GET['sd_id'])) {
  $colname_sd = $_GET['sd_id'];
}
mysql_select_db($database_book, $book);
$query_sd = sprintf("SELECT * 
  FROM tbl_schedule  as s, tbl_member as m
  WHERE sd_id = %s
  AND s.mem_id=m.mem_id", GetSQLValueString($colname_sd, "int"));
...
<form action="sd_form_edit_db.php" method="POST" enctype="multipart/form-data"  name="add" class="form-horizontal" id="add">
  ...
  <div class="col-sm-2" align="right">วัน</div>
    <div class="col-sm-5" align="left">
  	  <select name="sd_day" id="sd_day"  class="form-control">
  	    <option><?php echo $row_sd['sd_day']; ?></option>
  ...
  <div class="col-sm-2" align="right">พนักงาน</div>
       <div class="col-sm-5" align="left">
         ...
         <option value="<?php echo $row_sd['mem_id']?>"><?php echo $row_sd['mem_name']?></option>
...
<div class="form-group">
    ...
    <div class="col-sm-6">
      ...
      <input type="hidden" name="sd_id" value="<?php echo $row_sd['sd_id']; ?>">
    </div>
  </div>
</form>
<?php
